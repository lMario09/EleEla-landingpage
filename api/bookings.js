  ...
  try {
    ...
    if (req.method === 'DELETE') {
      ...
      if (result.rows.length === 0) {
        return res.writeHead(404, { ...headers, 'Content-Type': 'application/json' }).end(JSON.stringify({ error: 'Agendamento não encontrado' }))
      }
      return res.writeHead(200, { ...headers, 'Content-Type': 'application/json' }).end(JSON.stringify({ message: 'Agendamento excluído com sucesso' }))
    }
